...
// Accessors to private callbacks of GattCharacteristic
HIJACK_MEMBER(read_cb_access, FunctionPointerWithContext<GattReadAuthCallbackParams *> GattCharacteristic::*, &GattCharacteristic::readAuthorizationCallback);
HIJACK_MEMBER(write_cb_access, FunctionPointerWithContext<GattWriteAuthCallbackParams *> GattCharacteristic::*, &GattCharacteristic::writeAuthorizationCallback);
...
void GattServerMock::fake_register_services(GattService& gattService)
{
    ...
    for (size_t i = 0; i < gattService.getCharacteristicCount(); ++i) {
        ...
        c.read_cb = (ref.*read_cb_access);
        c.write_cb = (ref.*write_cb_access);
